...
export function calculateLatLonFromPointer(pointer, bgNode, calibration) {
  if (!pointer || !bgNode || !calibration?.topRight || !calibration?.bottomLeft) return undefined;
  const bx = bgNode.x();
  const by = bgNode.y();
  ...
  const lon = calibration.bottomLeft.lon + (relX / bw) * (calibration.topRight.lon - calibration.bottomLeft.lon);
  const lat = calibration.topRight.lat + (relY / bh) * (calibration.bottomLeft.lat - calibration.topRight.lat);
  return { lat, lon };
}
